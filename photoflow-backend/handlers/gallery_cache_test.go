...
func mintaFoto(t *testing.T, h *Handler, magicLink, query string) map[string]any {
	t.Helper()
...
func drivePunya(nama ...string) []storage.PhotoRef {
	out := make([]storage.PhotoRef, 0, len(nama))
	for _, n := range nama {
...
func TestGaleriMembacaDriveSaatBelumAdaSalinan(t *testing.T) {
	...
	body := mintaFoto(t, h, project.MagicLinkToken, "")

	if store.dibaca != 1 {
		t.Fatalf("Drive dibaca %d kali, seharusnya 1", store.dibaca)
	}
...
func TestGaleriTidakMembacaDriveSaatSalinanMasihSegar(t *testing.T) {
	db := newSubmitTestDB(t)
	project := seedProject(t, db, 0)
	store := &storePenghitung{files: drivePunya("a.jpg", "b.jpg")}
	h := handlerDenganStore(t, db, store)

	mintaFoto(t, h, project.MagicLinkToken, "") // mengisi salinan
	sesudahIsi := store.dibaca
	...
	if store.dibaca != sesudahIsi {
		t.Fatalf("Drive dibaca %d kali tambahan, seharusnya nol", store.dibaca-sesudahIsi)
	}
}

func TestSalinanLawasDitandaiStale(t *testing.T) {
	db := newSubmitTestDB(t)
	project := seedProject(t, db, 0)
	store := &storePenghitung{files: drivePunya("a.jpg")}
	h := handlerDenganStore(t, db, store)

	mintaFoto(t, h, project.MagicLinkToken, "")
	if body := mintaFoto(t, h, project.MagicLinkToken, ""); body["stale"] != false {
		t.Fatalf("salinan baru ditandai stale: %v", body["stale"])
	}

	setSyncedAt(t, db, project.ID, time.Now().Add(-umurSalinanMaks-time.Minute))
	if body := mintaFoto(t, h, project.MagicLinkToken, ""); body["stale"] != true {
		t.Fatalf("salinan lawas tidak ditandai stale")
	}
}

func TestPenyegaranDitolakSaatTerlaluBerdekatan(t *testing.T) {
	db := newSubmitTestDB(t)
	project := seedProject(t, db, 0)
	store := &storePenghitung{files: drivePunya("a.jpg")}
	h := handlerDenganStore(t, db, store)
	...
	for i := 0; i < 5; i++ {
		mintaFoto(t, h, project.MagicLinkToken, "?refresh=1")
	}
	if store.dibaca != sesudahIsi {
		t.Fatalf("Drive dibaca %d kali tambahan meski jeda belum lewat", store.dibaca-sesudahIsi)
	}

	setSyncedAt(t, db, project.ID, time.Now().Add(-jedaSegarMin-time.Second))
	mintaFoto(t, h, project.MagicLinkToken, "?refresh=1")
	if store.dibaca != sesudahIsi+1 {
		t.Fatalf("penyegaran sesudah jeda tidak membaca Drive")
	}
}

func TestPenyegaranMempertahankanPilihanKlien(t *testing.T) {
	db := newSubmitTestDB(t)
	project := seedProject(t, db, 0)
	...
	if len(terpilih) != 1 || terpilih[0] != "b.jpg" {
		t.Fatalf("pilihan sesudah penyegaran = %v, seharusnya [b.jpg]", terpilih)
	}

	var jumlah int64
	db.Model(&models.Photo{}).Where("project_id = ?", project.ID).Count(&jumlah)
	if jumlah != 4 {
		t.Fatalf("%d foto tersimpan, seharusnya 4", jumlah)
	}
}
